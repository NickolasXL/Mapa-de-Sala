...
class HomePrincipal(QMainWindow):
    ...
    def verificarPermissoes(self):
        ...
        tipo_conta = usuario.get("tipo", "comum")  # Pegando o tipo de conta, padrão para "comum"

        if tipo_conta not in ["admin", "suporte"]:
            self.btnPessoas.hide()
            self.btnPessoa.hide()
            self.btnEditarSimples.hide()
            self.btnEditar.hide()
        else:
            self.btnPessoas.show()
            self.btnPessoa.show()
            self.btnEditarSimples.show()
            self.btnEditar.show()
